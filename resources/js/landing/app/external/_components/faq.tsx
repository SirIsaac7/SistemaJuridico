...
function AccordionItem({ q, a, index }: { q: string; a: string; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="border-slate-900/10 border-b"
    >
      <button
        type="button"
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <p className="pb-6 text-slate-600 text-sm leading-relaxed">{a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
...
function CampoTexto({
  ...
  return (
    <div>
      <label htmlFor={id} className="mb-2 block font-semibold text-slate-500 text-xs uppercase tracking-wider">
        {etiqueta}
      </label>
      <div
      ...
      >
        {prefijo ? (
          <span className="border-slate-200 border-r py-3 pr-3 pl-4 text-slate-500 text-sm tabular-nums">
            {prefijo}
          </span>
        ) : null}
        <input
      ...
      {error ? (
        <p id={`${id}-error`} className="mt-1.5 text-red-500 text-xs">
          {error}
        </p>
      ) : null}
      {!error && ayuda ? <p className="mt-1.5 text-slate-500 text-xs">{ayuda}</p> : null}
    </div>
  );
}
...
export function Faq() {
  ...
  const errorDe = (campo: CampoConsulta) =>
    errores[campo] ?? (tocados[campo] ? (validaciones[campo] ?? undefined) : undefined);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!respuesta.ok) {
        ...
        toast.error(general ?? primero ?? "No se pudo enviar la consulta. Inténtelo nuevamente.");
        return;
      }
      ...
      toast.error("No se pudo enviar la consulta. Revise su conexión e inténtelo nuevamente.");
    } finally {
      setEnviando(false);
    }
  };
      ...
      <div className="lj-container relative z-10">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-20">
          ...
          <div>
            ...
            <FadeUp delay={0.1}>
              <h2 className="lj-font-heading mb-10 font-black text-3xl text-slate-900 uppercase sm:text-4xl md:text-5xl">
                Resolvemos
                <br />
          ...
          <SlideIn direction="right">
            <div className="lj-glass lj-glass-sheen relative rounded-3xl p-6 sm:p-8 md:p-10">
              <div className="relative z-10">
                ...
                <h3 className="lj-font-heading mb-2 font-black text-3xl text-slate-900 uppercase leading-tight md:text-4xl">
                  Solicite su
                  <br />
                  asesoría hoy
                </h3>
                ...
                {submitted ? (
                  ...
                  >
                    ...
                    <h4 className="lj-font-heading mb-2 font-black text-2xl text-slate-900 uppercase">
                      ¡Consulta enviada!
                    </h4>
                    <p className="text-slate-600 text-sm">
                      Un asesor jurídico se comunicará con usted en menos de 24 horas.
                    </p>
                  </motion.div>
                ) : (
                  <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                    ...
                    <div>
                      <label
                        htmlFor="lj-mensaje"
                        className="mb-2 block font-semibold text-slate-500 text-xs uppercase tracking-wider"
                      >
                        Su consulta
                      </label>
                      <textarea
                      ...
                      <div className="mt-1.5 flex items-start justify-between gap-3">
                        {errorDe("mensaje") ? (
                          <p id="lj-mensaje-error" className="text-red-500 text-xs">
                            {errorDe("mensaje")}
                          </p>
                        ) : (
                          <span />
                    ...
                    <motion.button
                      ...
                      className="lj-btn-shine relative flex w-full items-center justify-center gap-2 overflow-hidden rounded-full bg-[linear-gradient(100deg,#4f46e5_0%,#7c3aed_100%)] py-4 font-semibold text-[15px] text-white shadow-[0_14px_34px_-10px_rgba(79,70,229,0.75)] transition-all duration-200 hover:shadow-[0_18px_44px_-10px_rgba(79,70,229,0.9)] disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:shadow-none"
                    >
                      {enviando ? (
                        <>
                    ...
                    <p className="text-center text-slate-500 text-xs">
                      Su consulta queda registrada en el sistema y un asesor jurídico le responderá por WhatsApp o
                      correo.
                    </p>
                  </form>
                )}
