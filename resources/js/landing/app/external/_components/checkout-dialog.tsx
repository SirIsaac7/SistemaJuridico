...
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/landing/components/ui/dialog";
...
const claseInput =
  "w-full rounded-lg border border-slate-200 bg-white px-3.5 py-2.5 text-slate-900 text-sm placeholder:text-slate-400 outline-none transition-colors focus:border-indigo-500 disabled:opacity-50";
const claseLabel = "mb-1.5 block font-semibold text-slate-600 text-xs uppercase tracking-wider";
...
export function CheckoutDialog({ plan, periodicidadInicial, onClose }: CheckoutDialogProps) {
  ...
  const onSubmit = async (valores: ValoresCheckout) => {
    ...
    const { identificador } = (datos ?? {}) as { identificador?: string };
    if (!identificador) {
      setErrorEnvio("No se pudo registrar el cobro. Inténtalo nuevamente en unos minutos.");
      return;
    }
    // El QR de cobro se muestra en nuestra propia página de pago, que además
    // sondea el estado hasta que el banco confirma la transacción.
    window.location.assign(`/planes/pago/${identificador}`);
  };
  ...
  return (
    <Dialog open={plan !== null} onOpenChange={(abierto) => !abierto && !enviando && cerrar()}>
      <DialogContent
        ...
        className="max-h-[92vh] gap-0 overflow-y-auto border-slate-200 bg-white p-0 text-slate-900 shadow-[0_30px_80px_-20px_rgba(30,41,99,0.45)] sm:max-w-lg"
        showCloseButton={!enviando}
      >
        {/* Encabezado con el resumen del plan elegido */}
        <div className="border-slate-200 border-b bg-[linear-gradient(135deg,#eef4ff_0%,#f8fafc_100%)] px-6 py-5">
          <DialogHeader className="gap-1 text-left">
            <DialogTitle className="lj-font-heading font-black text-slate-900 text-xl">
              {paso === 1 ? "Registro de datos del cliente" : "Datos de facturación"}
            </DialogTitle>
            <DialogDescription className="text-slate-600 text-sm">
              {plan ? (
                <>
                  Plan <span className="font-semibold text-indigo-600">{plan.nombre}</span>
                  {precioActual ? (
                    <>
                      {" · "}
            ...
            {[1, 2].map((n) => (
              <div
                ...
                className={`h-1.5 flex-1 rounded-full transition-colors ${
                  paso >= n ? "bg-[linear-gradient(90deg,#4f46e5,#7c3aed)]" : "bg-slate-200"
                }`}
              />
            ))}
          </div>
            ...
            <>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {plan.precios.map((precio) => {
                          ...
                          return (
                            ...
                            >
                              <span className="block font-bold text-slate-900 text-sm">{precio.etiqueta}</span>
                              <span className="block text-slate-600 text-xs">
                                Bs {precio.precio} {precio.sufijo}
                              </span>
                              {ahorro > 0 && (
                                <span className="mt-1 inline-block rounded-full bg-indigo-100 px-2 py-0.5 font-semibold text-[10px] text-indigo-700">
                                  Ahorras Bs {ahorro}
                                </span>
                              )}
                            </button>
              ...
              <div>
                <label htmlFor="co-canal" className={claseLabel}>
                  ¿Por qué medio te enteraste de Normativa Virtual?
                </label>
              ...
              <button
                type="button"
                onClick={avanzarPaso}
                className="mt-1 flex w-full items-center justify-center gap-2 rounded-xl bg-[linear-gradient(100deg,#4f46e5_0%,#7c3aed_100%)] py-3.5 text-center font-semibold text-[15px] text-white shadow-[0_14px_34px_-10px_rgba(79,70,229,0.75)] transition-all hover:shadow-[0_18px_44px_-10px_rgba(79,70,229,0.9)]"
              >
          ...
          {paso === 2 && plan && precioActual && (
            <>
              ...
              <label
                ...
                className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5"
              >
                <input
                ...
                />
                <span>
                  <span className="block font-semibold text-slate-900 text-sm">Deseo factura con mis datos</span>
                  <span className="block text-slate-600 text-xs">
              ...
              {quiereFactura && (
                <div className="flex flex-col gap-4 rounded-xl border border-indigo-200 bg-indigo-50/70 p-4">
                  <div>
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <p className="mb-3 font-semibold text-slate-600 text-xs uppercase tracking-wider">Resumen del pedido</p>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-600">
                    Plan {plan.nombre} · {precioActual.etiqueta} · {plan.dispositivos}
                  </span>
                  <span className="font-bold text-slate-900">Bs {precioActual.precio}</span>
                </div>
                <div className="mt-3 flex items-center justify-between border-slate-200 border-t pt-3">
                  <span className="font-semibold text-slate-900 text-sm">Total a pagar</span>
                  <span className="lj-font-heading lj-gradient-indigo font-black text-2xl">
                    Bs {precioActual.precio}
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  onClick={() => setPaso(1)}
                  disabled={enviando}
                  className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3.5 font-semibold text-slate-700 text-sm transition-colors hover:border-indigo-300 hover:text-indigo-600 disabled:opacity-50"
                >
                ...
                <button
                  type="submit"
                  disabled={enviando}
                  className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[linear-gradient(100deg,#4f46e5_0%,#7c3aed_100%)] py-3.5 text-center font-semibold text-[15px] text-white shadow-[0_14px_34px_-10px_rgba(79,70,229,0.75)] transition-all hover:shadow-[0_18px_44px_-10px_rgba(79,70,229,0.9)] disabled:opacity-60"
                >
                  {enviando ? (
                    <>
                      <Loader2 size={16} className="animate-spin" /> Generando tu QR de pago…
                    </>
                  ) : (
                    <>
                      <Lock size={14} /> Pagar Bs {precioActual.precio} con QR
                    </>
                  )}
                </button>
              </div>

              <p className="flex items-center justify-center gap-1.5 text-center text-slate-500 text-xs">
                <ShieldCheck size={13} className="text-indigo-600" />
                Cobro seguro por QR del Banco Nacional de Bolivia: pagas desde la app de tu banco.
              </p>
            </>
          )}
        </form>
