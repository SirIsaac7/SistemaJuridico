  ...
  return (
    <div className="mb-3 flex gap-0.5">
      {Array.from({ length: count }, (_, i) => i + 1).map((n) => (
        <Star key={n} size={12} className="text-indigo-500" fill="#6366f1" />
      ))}
    </div>
  );
}
...
export function Testimonials() {
  return (
    <section id="testimonios" className="lj-section bg-[linear-gradient(180deg,#f8fafc_0%,#eef4ff_100%)]">
        ...
        <div className="mb-10 text-center md:mb-16">
          ...
          <FadeUp delay={0.1}>
            <h2 className="lj-font-heading font-black text-3xl text-slate-900 uppercase sm:text-4xl md:text-5xl">
              Lo que dicen
              <br />
              <span className="lj-gradient-indigo">nuestros clientes</span>
            </h2>
          </FadeUp>
        </div>
        ...
        <div className="columns-1 gap-6 space-y-6 md:columns-2 lg:columns-3">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              className="lj-glass lj-glass-sheen group relative break-inside-avoid rounded-3xl p-6 transition-shadow duration-300 hover:shadow-[0_32px_70px_-24px_rgba(30,41,99,0.45)]"
            >
              <div className="relative z-10">
                <StarRating count={t.rating} />
                <p className="mb-6 text-slate-600 text-sm leading-relaxed">&ldquo;{t.quote}&rdquo;</p>
                <div className="flex items-center gap-3">
                  <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full border border-white/80 shadow-sm">
                    <Image src={t.image} alt={t.name} fill className="object-cover" sizes="40px" />
                  </div>
                  <div>
                    <p className="lj-font-heading font-bold text-slate-900 text-sm">{t.name}</p>
